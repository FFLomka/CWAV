...
function oClockInit(){
	...
	oClock.style.lineHeight = height+'px'
...
function getTime(){
    ...
    if(tStyle){
		if(tShowSencends){
			oClock.innerHTML = add0(c.getHours())+' <span style="opacity: ' + (timessss ? 0 : 1) + ';">' +dot+ "</span> "+add0(c.getMinutes())+" <span class='sec'>"+add0(c.getSeconds()) + "</span>"
			...
			oClock.innerHTML = add0(c.getHours())+' <span style="opacity: ' + (timessss ? 0 : 1) + ';">' +dot+ "</span> "+add0(c.getMinutes())
        ...
        h = h<=12 ? h : h-12
		if(tShowSencends){
			oClock.innerHTML = "<span id='time'>"+add0(h)+' <span style="opacity: ' + (timessss ? 0 : 1) + ';">' +dot+ "</span> "+add0(c.getMinutes())+" <span class='sec'>"+add0(c.getSeconds())+"</span><span class='st'>"+str+"</span></span>"
			realTime =  add0(h) + ' '+dot+' '  + add0(c.getMinutes()) + ' '+dot+' ' +  add0(c.getSeconds()) + ' ' + str
			ar = [ h, c.getMinutes(), c.getSeconds()].reduce(padClock, '')
			oSlide_dots.style.display = 'inline-block'
		}else{
			oClock.innerHTML = "<span id='time'>"+add0(h)+' <span style="opacity: ' + (timessss ? 0 : 1) + ';">' +dot+ "</span> "+add0(c.getMinutes())+ "</span>" +" <span class='sec'>"+ str + "</span>"
